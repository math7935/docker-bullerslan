...
if ! [[ -f 'bullerslan-server-files-0.1.0.zip' ]]; then
	rm -fr defaultconfigs config kubejs libraries mods Simple.zip forge*
	curl -Lo 'bullerslan-server-files-0.1.0.zip' 'https://edge.forgecdn.net/files/4875/747/bullerslan-server-files-0.1.0.zip' || exit 9
	unzip -u -o 'bullerslan-server-files-0.1.0.zip' -d /data
